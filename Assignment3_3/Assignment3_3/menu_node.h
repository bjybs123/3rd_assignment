...
void Link::insert(char* nameIn, int priceIn)
{
	...
	menu_node* tempNode = pHead;					//임시 노드에 pHead가 가르키는 초기 노드를 저장.

	if (pHead == nullptr)				//임시노드가 가르키는 노드가 비어있으면 새로운노드를 저장
	{
	...
	else
	{
		while (tempNode != nullptr)
		{
			if (tempNode->getPrice() > priceIn)					//만약에 현재 노드의 가격이 새로 입력된 가격 보다 크다면
			{
				...
				{
					menu_node* prev = tempNode->getPrev();
					currentNode->setNext(tempNode);
			...
			else if (tempNode->getPrice() < priceIn)			//만약에 현재 노드의 가격이 새로 입력된 가격 보다 작다면
			{
				...
				{
					if (tempNode->getNext()->getPrice() < priceIn)
					{
...
void Link::print()
{
	menu_node* tempNode = pHead;						//임시 노드를 만들어서 pHead가 가르키는 첫번째 노드를 저장해준다.
...
{
	menu_node* tempNode = pHead;						//pHead의 첫번째 노드를 저장.
	...
	cout << "Not in menu\n";							//while문에서 함수종료가 이루어지지않았다면 문자열이 같은 노드가 
														//존재하지 않기때문에 존재하지 않다고 출력하고 함수를 종료한다.
	return;
}
void Link::delNode(char* delName)
{
	menu_node* tempNode = pHead;			//개별적으로 검사 할 임시 노드 생성 pHead의 첫번째 노드를 저장.
	...
	while (tempNode != nullptr)						
	{
		...
		{
			menu_node* delNode = tempNode;
			if (tempNode->getPrev() == nullptr)					//첫 노드인 경우
			{
				pHead = tempNode->getNext();
				delete delNode;
				return;
			}
			else
			{
				menu_node* prev = tempNode->getPrev();
				menu_node* next = tempNode->getNext();
				prev->setNext(next);
				if (next != nullptr)
					next->setPrev(prev);
				delete delNode;
